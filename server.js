...
function parseDXSpiderSpotLine(line) {
  ...
  const tableMatch = line.match(/^\s*(\d+\.?\d*)\s+([A-Z0-9\/\-]+)\s+\d{1,2}-[A-Za-z]{3}-\d{4}\s+(\d{4})Z\s+(.*)\s+<([A-Z0-9\/\-]+)>\s*$/i);
  if (tableMatch) {
...
function addCustomSessionSpot(session, spot) {
  ...
  const duplicate = session.spots.some(s =>
    s.call === spot.call &&
    s.freq === spot.freq &&
...
function connectCustomSession(session) {
  ...
  client.connect(session.node.port, session.node.host, () => {
    ...
    logDebug(`[DX Cluster] DX Spider: connected to ${session.node.host}:${session.node.port} as ${session.loginCallsign}`);
  ...
  client.on('data', (data) => {
    ...
    if (session.loginSent && !session.commandSent && (
      ...
      setTimeout(() => {
        if (session.client && session.connected) {
          if (!session.initialSnapshotDone) {
            logInfo(`[DX Cluster] Sending command: sh/dx 25 to ${session.node.host}:${session.node.port} as ${session.loginCallsign}`);
...
function getDxPathsCache(cacheKey) {
  if (!dxSpotPathsCacheByKey.has(cacheKey)) {
    dxSpotPathsCacheByKey.set(cacheKey, { paths: [], allPaths: [], timestamp: 0 });
  }
...
app.get('/api/dxcluster/paths', async (req, res) => {
  ...
  try {
    ...
    if (source === 'custom' && customHost) {
      logDebug(`[DX Paths] Using custom telnet session: ${customHost}:${customPort} as ${getDxClusterLoginCallsign(userCallsign)}`);
    ...
    const existingValidPaths = pathsCache.allPaths.filter(p =>
